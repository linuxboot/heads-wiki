...
### HOTP (One-time passwords generated by Heads)
**Heads generates HOTP codes** to verify that firmware hasn't been tampered with. When Heads starts up, it creates a code and sends it to your USB Security dongle. The dongle checks if this code is correct. If it matches, your firmware is verified as safe.

**What you need**: A compatible USB Security dongle (see [Prerequisites compatibility table](/Installing-and-Configuring/Prerequisites#compatible-usb-security-dongles)). Many dongles like YubiKey 5 Series cannot do this verification check that Heads requires.

### TOTP (Time-based codes generated by Heads)  
**Heads generates TOTP codes** that appear on your screen. You compare this code with the one shown in your phone's authenticator app. If they match, your firmware is verified as safe.

**What you need**: 
- A smartphone with an authenticator app
- **Both devices must show the same time** - Heads requires manual time setting through Options menu to UTC/GMT timezone. Phones sync time automatically, but Heads cannot.

**Time sync is critical**: If the times don't match between your phone and Heads, the codes will be different and verification will fail.

### Which verification method to use:
- **HOTP with USB Security dongle**: More reliable, works without time synchronization
- **TOTP with phone**: Requires accurate time setting, can be challenging on systems that haven't been used for a while
