...
**Important**: Heads requires USB Security dongles that support OpenPGP smartcard functionality for signing `/boot` contents. Reverse HOTP verification is optional but strongly recommended for remote attestation. Users can choose to use board configurations without HOTP enforcement, but not all boards offer both HOTP and non-HOTP variants. Some vendors only support HOTP-enabled configurations in Heads.
...
2. **Reverse HOTP support**: Required for firmware verification (this is NOT the same as standard HOTP)
3. **TPMTOTP**: This is generated by Heads itself using the TPM and displayed on screen - no USB dongle needed

### Compatibility differences:

**Why some dongles aren't compatible**: Not all USB Security dongles support the specialized firmware required for Heads' reverse HOTP verification. For detailed technical specifications, refer to the [Nitrokey documentation](https://docs.nitrokey.com/) and [Librem Key documentation](https://puri.sm/posts/librem-key-faq/) for what makes these dongles compatible with Heads.
