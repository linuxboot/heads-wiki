...
**Important**: Heads requires USB Security dongles that support OpenPGP smartcard functionality for signing `/boot` contents. HOTP verification is optional but strongly recommended for remote attestation. Users can choose to use board configurations without HOTP enforcement, but not all boards offer both HOTP and non-HOTP variants. Some vendors only support HOTP-enabled configurations in Heads.

### What Heads needs from your USB Security dongle:

1. **OpenPGP smartcard support**: Required for signing `/boot` contents with your private key
2. **HOTP verification support**: Required for firmware verification (this is NOT the same as standard HOTP)
3. **TPMTOTP**: This is generated by Heads itself using the TPM and displayed on screen - no USB dongle needed

### Compatibility differences:

**Why some dongles aren't compatible**: Not all USB Security dongles support HOTP verification (also called "reverse HOTP"), which Heads requires for firmware verification. For technical details, see the [Nitrokey HOTP verification documentation](https://github.com/Nitrokey/nitrokey-hotp-verification) and [Nitrokey SDK documentation](https://docs.nitrokey.com/software/nitrokey-sdk-py/api/nitrokey.nk3.secrets_app#nitrokey.nk3.secrets_app.SecretsApp.verify_code).
