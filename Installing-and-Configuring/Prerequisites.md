...
**Important**: Heads requires USB Security dongles that support **both** OpenPGP smartcard functionality AND reverse HOTP verification. These are specialized requirements that differ from standard HOTP/TOTP support.
...
- **Standard HOTP/TOTP**: Supported by many dongles (including YubiKey 5 Series) for general authentication
- **Reverse HOTP**: A specialized protocol required by Heads, only supported by specific dongles listed below
- **TPMTOTP**: Generated by Heads using TPM measurements, displayed on screen for manual verification against your phone
